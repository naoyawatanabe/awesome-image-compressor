...
/***/ "./src/compress-assets.js":
/*!********************************!*\
  !*** ./src/compress-assets.js ***!
  \********************************/
/*! exports provided: onExportSlices */
...
function compressSlices(exportRequests, fileType) {
  var targetPaths = exportRequests.filter(function (currentExport) {
    return currentExport.request.format() == fileType.extension;
  }).map(function (currentExport) {
    return String(currentExport.path);
  }).map(function (currentExport) {
    return String(currentExport.replace(/\\/g, '\\\\'));
  });
  ...
  var targetDesc = "".concat(targetPaths.length, " ").concat(fileType.extension, " file").concat(targetPaths.length == 1 ? '' : 's');
  execSync("".concat(fileType.compressor, " \"").concat(targetPaths.join('" "'), "\""));
  UI.message("Compressed ".concat(targetDesc));
}

function pngquant() {
  var pngquantPath = context.plugin.urlForResourceNamed('node_modules/pngquant-bin/vendor/pngquant').path();
  return pngquantPath.replace(/ /g, '\\ ') + ' --ext .png -f';
}

function jpegoptim() {
  var jpegoptimPath = context.plugin.urlForResourceNamed('node_modules/jpegoptim-bin/vendor/jpegoptim').path();
